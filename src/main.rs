...
fn main() {
    ...
    match args.command {
        Some(Commands::Crispr(crispr_args)) => {
            crispr(crispr_args);
        }
        None => {
            // Default behavior - parse as DefaultArgs
                    ...
                    scope.spawn(|| {
                            ...
                            && let Some(record) = guard.next()
                        {
                            ...
                            let matches = match args.alphabet {
                                ...
                                Alphabet::Dna => {
                                    search_maybe_rc::<Dna>(query, text, args.k, args.rc)
                                }
                                Alphabet::Iupac => {
                                    search_maybe_rc::<Iupac>(query, text, args.k, args.rc)
                                }
                            };
